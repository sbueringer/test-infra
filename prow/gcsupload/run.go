...
func (o Options) Run(spec *downwardapi.JobSpec, extra map[string]iov2.UploadFunc) error {
...
func (o Options) assembleTargets(spec *downwardapi.JobSpec, extra map[string]iov2.UploadFunc) map[string]iov2.UploadFunc {
	jobBasePath, gcsPath, builder := PathsForJob(o.GCSConfiguration, spec, o.SubDir)

	uploadTargets := map[string]iov2.UploadFunc{}
	...
	if o.LocalOutputDir == "" {
		...
		if alias := gcs.AliasForSpec(spec); alias != "" {
			uploadTargets[iov2.JoinStoragePath(o.GCSConfiguration.Path,alias)] = iov2.DataUpload(strings.NewReader(jobBasePath), &blob.Attributes{
		...
		if latestBuilds := gcs.LatestBuildForSpec(spec, builder); len(latestBuilds) > 0 {
			for _, latestBuild := range latestBuilds {
				...
				uploadTargets[iov2.JoinStoragePath(o.GCSConfiguration.Path,path.Join(dir, metadataFromFileName))] = iov2.DataUpload(strings.NewReader(spec.BuildID), attrs)
			}
		}
	} else {
	...
	for _, item := range o.Items {
		...
		if info.IsDir() {
			...
			uploadTargets[destination] = iov2.FileUpload(item, attrs)
		}
	}

	for destination, upload := range extra {
		uploadTargets[iov2.JoinStoragePath(gcsPath, destination)] = upload
	}
...
func PathsForJob(options *prowapi.GCSConfiguration, spec *downwardapi.JobSpec, subdir string) (string, string, gcs.RepoPathBuilder) {
	...
	jobBasePath = iov2.JoinStoragePath(options.Path, jobBasePath)
	var gcsPath string
	if subdir == "" {
		gcsPath = jobBasePath
	} else {
		gcsPath = iov2.JoinStoragePath(jobBasePath, subdir)
	}
...
func gatherArtifacts(artifactDir, gcsPath, subDir string, uploadTargets map[string]iov2.UploadFunc) {
	...
	filepath.Walk(artifactDir, func(fspath string, info os.FileInfo, err error) error {
		...
		if relPath, err := filepath.Rel(artifactDir, fspath); err == nil {
			...
			uploadTargets[destination] = iov2.FileUpload(fspath, attrs)
		} else {
