...
func readLatestBuild(opener iov2.Opener, root string) (int64, error) {
	key := iov2.JoinStoragePath(root, latestBuildFile)
...
func resolveSymLink(opener iov2.Opener, symLink string) (string, error) {
...
func spyglassLink(opener iov2.Opener, storagePath, id string) (string, error) {
	p, err := getPath(opener, storagePath, id, "")
	if err != nil {
		return "", fmt.Errorf("failed to get path: %v", err)
	}
	return path.Join(spyglassPrefix, providers.EncodeStorageURL(p)), nil
}

func getPath(opener iov2.Opener, root, id, fname string) (string, error) {
	_, _, relativePath, err := providers.ParseStoragePath(root)
	if err != nil {
		return "", err
	}
	...
	symLink := iov2.JoinStoragePath(root, id+".txt")
	dir, err := resolveSymLink(opener, symLink)
	if err != nil {
		return "", fmt.Errorf("failed to resolve sym link: %v", err)
	}
	return iov2.JoinStoragePath(dir, fname), nil
}

// reads specified JSON file in to `data`
func readJSON(opener iov2.Opener, key string, data interface{}) error {
...
func listBuildIDs(opener iov2.Opener, root string) ([]int64, error) {
	ids := []int64{}
...
func parseJobHistURL(url *url.URL) (path string, buildID int64, err error) {
	...
	path = providers.DecodeStorageURL(path)
...
func getBuildData(opener iov2.Opener, dir string) (buildData, error) {
	b := buildData{
		Result:     "Unknown",
		commitHash: "Unknown",
	}
	started := gcs.Started{}
	err := readJSON(opener, iov2.JoinStoragePath(dir, "started.json"), &started)
	if err != nil {
	...
	err = readJSON(opener, iov2.JoinStoragePath(dir, "finished.json"), &finished)
	if err != nil {
		b.Result = "Pending"
...
func getJobHistory(url *url.URL, opener iov2.Opener) (jobHistoryTemplate, error) {
	start := time.Now()
