...
function redrawQueries(): void {
    ...
    for (let i = 0; i < tideData.Queries.length; i++) {
        ...
        const a = createLink(
            `https://git.daimler.com/search?utf8=${encodeURIComponent("\u2713")}&q=${encodeURIComponent(query)}`,
            "GitHub Search Link",
        );
        li.appendChild(a);
        ...
        if (orgs.length > 0) {
            ...
            for (let j = 0; j < orgs.length; j++) {
                innerLi.appendChild(createLink("https://git.daimler.com/" + orgs[j], orgs[j]));
                if (j + 1 < repos.length) {
        ...
        if (repos.length > 0) {
            ...
            for (let j = 0; j < repos.length; j++) {
                innerLi.appendChild(createLink("https://git.daimler.com/" + repos[j], repos[j]));
                if (j + 1 < repos.length) {
                    innerLi.appendChild(document.createTextNode(", "));
                }
            }
        ...
        if (excludedRepos.length > 0) {
            ...
            for (let j = 0; j < excludedRepos.length; j++) {
                innerLi.appendChild(createLink("https://git.daimler.com/" + excludedRepos[j], excludedRepos[j]));
...
function createRepoCell(pool: TidePool): HTMLTableDataCellElement {
    ...
    const branchLink = `https://git.daimler.com/${pool.Org}/${pool.Repo}/tree/${pool.Branch}`;
...
function addPRsToElem(elem: HTMLElement, pool: TidePool, prs?: PullRequest[]): void {
    if (prs) {
        for (let i = 0; i < prs.length; i++) {
            const a = document.createElement("a");
            a.href = `https://git.daimler.com/${pool.Org}/${pool.Repo}/pull/${prs[i].Number}`;
