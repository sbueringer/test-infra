...
export namespace cell {
  ...
  export function commitRevision(repo: string, ref: string, SHA: string, pushCommitLink: string): HTMLTableDataCellElement {
    ...
    if (!bl.href) {
        bl.href = `https://git.daimler.com/${repo}/commit/${SHA}`;
    }
  ...
  export function addPRRevision(elem: Node, repo: string, pull: Pull): void {
    ...
    if (pull.link) {
      pl.href = pull.link;
        ...
        pl.href = `https://git.daimler.com/${repo}/pull/${pull.number}`;
    }
    ...
    if (pull.sha) {
      ...
      if (pull.commit_link) {
        cl.href = pull.commit_link;
          ...
          cl.href = `https://git.daimler.com/${repo}/pull/${pull.number}/commits/${pull.sha}`;
      }
    ...
    if (pull.author) {
      ...
      if (pull.author_link) {
        al.href = pull.author_link;
          ...
          al.href = "https://git.daimler.com/" + pull.author;
      }
      al.text = pull.author;
      elem.appendChild(al);
    }
