...
func githubPRLink(org, repo string, pr int) string {
	return fmt.Sprintf("https://git.daimler.com/%s/%s/pull/%d", org, repo, pr)
}

func githubCommitLink(org, repo, commitHash string) string {
	return fmt.Sprintf("https://git.daimler.com/%s/%s/commit/%s", org, repo, commitHash)
}

func jobHistLink(storagePath, jobName string) string {
	return fmt.Sprintf("/job-history/%s/pr-logs/directory/%s", prowio.EncodeStorageURL(storagePath), jobName)
}
...
func listJobBuilds(opener prowio.Opener, jobPrefixes []string) []jobBuilds {
...
func getPRBuildData(opener prowio.Opener, jobs []jobBuilds) []buildData {
	...
	for _, job := range jobs {
		for j, buildPrefix := range job.buildPrefixes {
			go func(j int, jobName, buildPrefix string) {
				...
				build.SpyglassLink = path.Join(spyglassPrefix, prowio.EncodeStorageURL(buildPrefix))
...
func getPRHistory(url *url.URL, config *config.Config, opener prowio.Opener, gitHubClient deckGitHubClient, gitClient git.ClientFactory) (prHistoryTemplate, error) {
	start := time.Now()
