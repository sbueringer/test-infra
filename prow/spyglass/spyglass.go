...
func New(ja *jobs.JobAgent, cfg config.Getter, opener iov2.Opener, ctx context.Context) *Spyglass {
	return &Spyglass{
		JobAgent:              ja,
		config:                cfg,
...
func (sg *Spyglass) ResolveSymlink(src string) (string, error) {
	...
	case providers.URLHasStorageProviderPrefix(src):
		src = providers.DecodeStorageURL(src)
		...
		// FIXME: test somehow
		return providers.DecodeStorageURL(path.Join(keyType, u.Host, u.Path)), nil
	default:
...
func (sg *Spyglass) JobPath(src string) (string, error) {
	src = strings.TrimSuffix(src, "/")
	keyType, key, err := splitSrc(providers.EncodeStorageURL(src))
	if err != nil {
		return "", fmt.Errorf("error parsing src: %v", src)
	}
	split := strings.Split(key, "/")
	switch {
	case providers.PathHasStorageProviderPrefix(src):
		if len(split) < 4 {
